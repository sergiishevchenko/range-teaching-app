...
function BubbleGraph({ graphData, points, onPointUpdate, challenge }) {
	...
	const [dragStart, setDragStart] = useState({
		...
		pointX: 0,
		pointY: 0,
	}); // x, y - mouse's coordinates; pointX, pointY - point's coordinates
	...
	const isPointOutOfRange = useCallback(
		(point, challenge) => {
	...
	const renderCustomShape = useCallback(
		(props) => {
			...
			const outOfRange = challenge
				? isPointOutOfRange(point, challenge)
				: false;
	...
	return (
		<div className={styles["bubble-graph-container"]}>
								...
								ticks={Array.from(
									{
										length:
											(graphData.xAxis.max -
												graphData.xAxis.min) /
												graphData.xAxis.step +
											1,
									},
									(_, i) =>
										graphData.xAxis.min +
										i * graphData.xAxis.step
								)}
								label={{
									value: graphData.xAxis.label,
								...
								ticks={Array.from(
									{
										length:
											(graphData.yAxis.max -
												graphData.yAxis.min) /
												graphData.yAxis.step +
											1,
									},
									(_, i) =>
										graphData.yAxis.min +
										i * graphData.yAxis.step
								)}
								label={{
						...
						{graphData.bubbleSize.values.map((value, index) => (
							<div key={index} className={styles["legend-item"]}>
								<div
									...
									style={{
										width: getBubbleSize(value),
										height: getBubbleSize(value),
									}}
								/>
								<span>{value}</span>
							</div>
			...
			<div className={styles["graph-instructions"]}>
				<p>
					💡 Click and drag the bubbles to move them. Adjust the
					y-axis positions to change the range.
				</p>
			</div>
		</div>
	);
